/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   map_parsing.c                                      :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: leramos- <leramos-@student.42lisboa.com    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/21 13:32:48 by leramos-          #+#    #+#             */
/*   Updated: 2025/11/24 15:00:38 by leramos-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

static char	**init_grid(char *file_name, int grid_height)
{
	char	**grid;
	int		fd;
	char	*line;
	int		i;

	grid = (char **)malloc(sizeof(char *) * (grid_height + 1));
	if (!grid)
		return (NULL);
	fd = open(file_name, O_RDONLY);
	if (fd < 0)
		return (NULL);
	i = 0;
	while (1)
	{
		line = get_next_line(fd);
		if (line == NULL)
			break ;
		trim_newline(line);
		grid[i] = line;
		i++;
	}
	grid[i] = NULL;
	close(fd);
	return (grid);
}

static void	scan_map_elements(t_map *map)
{
	int	i;
	int	j;

	map->collectibles = 0;
	i = 0;
	while (i < map->height)
	{
		j = 0;
		while (j < map->width)
		{
			if (map->grid[i][j] == PLAYER)
			{
				map->player.x = j;
				map->player.y = i;
			}
			if (map->grid[i][j] == COLLECTIBLE)
				map->collectibles++;
			j++;
		}
		i++;
	}
}

void	parse_map(t_data *data, int argc, char **argv)
{
	if (argc != 2)
		cleanup_and_exit(ERR_INVALID_ARG_COUNT, "Invalid argument count", data);
	if (!is_valid_extension(argv[1], "ber"))
		cleanup_and_exit(ERR_INVALID_EXT, "File extension isn't ber", data);
	data->map.height = get_line_count(argv[1]);
	if (data->map.height == -1)
		cleanup_and_exit(ERR_CANT_READ_FILE, "Couldn't read file", data);
	if (data->map.height == 0)
		cleanup_and_exit(ERR_EMPTY_FILE, "Map is empty", data);
	data->map.grid = init_grid(argv[1], data->map.height);
	if (!data->map.grid)
		cleanup_and_exit(ERR_CANT_INIT_MAP, "Couldn't initialize map", data);
	data->map.width = ft_strlen(data->map.grid[0]);
	scan_map_elements(&(data->map));
	if (!is_map_valid(data->map))
		cleanup_and_exit(ERR_INVALID_MAP, "Map is invalid", data);
	data->map.player.collected = 0;
	data->map.player.move_count = 0;
}
